                ...
                <div className={`page-top-margin list-item flex-direction-column w100 ${props.className || ''}`}>
                    {
                        props.list.map((item: { [K: string]: any }, index: number) => {
                            ...
                            if (!Utils.isBlank(searchText)) {
                                name = name.replaceAll(searchText || '', <span
                                    className="search-text">{searchText || ''}</span>)
                            }
                            return (
                                <div className="item card card-no-margin-lr card-shadow card-radius flex w100"
                                     key={index}>
                                    ...
                                    <div className="item-right h100 flex-1 overflow-hidden">
                                        ...
                                        <div
                                            className="card-desc card-top-margin over-ellipsis">{item.vod_year}/{item.vod_area}/{item.vod_class}</div>
                                        {
                                            !Utils.isBlank(item.vod_blurb) && (<div
                                                className="card-desc card-top-margin over-two-ellipsis">{item.vod_blurb || ''}</div>)
                                        }
