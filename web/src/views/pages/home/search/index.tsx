...
const Search: React.FC<IRouterProps> = (props: IRouterProps): ReactElement => {
    ...
    const hasData = () => {
        if (Utils.isObjectNull(homeStore.search)) return false
        if (homeStore.search.all.list.length > 0 ||
            homeStore.search.dramaSeries.list.length > 0 ||
            homeStore.search.movie.list.length > 0 ||
            homeStore.search.cartoon.list.length > 0 ||
            homeStore.search.variety.list.length > 0 ||
            homeStore.search.children.list.length > 0 ||
            homeStore.search.record.list.length > 0
        ) {
            return true
        }
            ...
            <div className="search-page flex-direction-column wh100 mark">
                <div className="page-wrapper wh100 flex-direction-column">
                    <div className="search-box flex-align-center">
                        <div className="svg-box back flex-center cursor-pointer"
                             onClick={() => {
