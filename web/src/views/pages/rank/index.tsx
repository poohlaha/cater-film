...
const Rank: React.FC<IRouterProps> = (props: IRouterProps): ReactElement => {
  ...
  const getComponentsHtml = (key: string = '') => {
    ...
    if (key === rankStore.tabsList[0].key) {
      return <List list={rankStore.hotJList} name={rankStore.tabsList[0].key} />
    }

    // 热播影
    if (key === rankStore.tabsList[1].key) {
      return <List list={rankStore.hotMList} name={rankStore.tabsList[1].key} />
    }

    // 热播漫
    if (key === rankStore.tabsList[2].key) {
      return <List list={rankStore.hotCList} name={rankStore.tabsList[2].key} />
    }
